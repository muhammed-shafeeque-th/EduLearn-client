...
export const useFileUpload = () => {
  ...
  const uploadMultipart = async (
    ...
    completeMultipartUpload(uploadId, multipartStateRef.current.completedParts);
      ...
      try {
        ...
        await apiClient.put(uploadUrl, file, {
          ...
          onUploadProgress: (progressEvent: AxiosProgressEvent) => {
            if (progressEvent.total && !uploadStateRef.current.isPaused) {
              ...
              const progressInfo: UploadProgressInfo = {
                ...
                speed: uploadStateRef.current.speed,
                eta: uploadStateRef.current.eta,
                isMultipart: false,
              };
              ...
              if (uploadStateRef.current.uploadId && progress % 10 === 0) {
                saveUploadProgress(uploadStateRef.current.uploadId, { progress, uploadedBytes });
              }
            }
          },
        });
  ...
  const uploadFile = useCallback(
    async (options: UploadOptions): Promise<void> => {
      ...
      try {
        ...
        if (error.message.includes('Network Error') || error.code === 'ECONNABORTED') {
          if (refreshCountRef.current < RETRIES_THRUSH_HOLD) {
            refreshCountRef.current++;

            const delay = AUTO_RESUME_DELAY * Math.pow(2, uploadStateRef.current.retryCount);
    ...
    // Dependencies are stable callbacks/refs; exhaustive-deps lint is noisy here.
...
/**
 * // hooks/use-file-upload.ts - COMPLETE REFACTOR
...
export const useFileUpload = () => {
  ...
  const stateRef = useRef<UploadState>(initialState);
  ...
  const calculateSpeed = useCallback((): number => {
    const now = Date.now();
    const timeDiff = now - timingRef.current.lastUpdate;

    if (timeDiff < 500) return stateRef.current.speed;

    const bytesDiff =
      stateRef.current.uploadedBytes - timingRef.current.lastBytes;
    const currentSpeed = (bytesDiff / timeDiff) * 1000;

    speedSamplesRef.current.push(currentSpeed);
    if (speedSamplesRef.current.length > SPEED_WINDOW) {
      speedSamplesRef.current.shift();
    }
    ...
    timingRef.current.lastUpdate = now;
    timingRef.current.lastBytes = stateRef.current.uploadedBytes;

    return Math.round(avgSpeed);
  }, []);

  // Calculate ETA
  const calculateEta = useCallback((speed: number): number => {
    if (speed <= 0) return 0;
    const remaining =
      stateRef.current.totalBytes - stateRef.current.uploadedBytes;
  ...
  const uploadFile = useCallback(
    async (options: {
      file: File;
      courseId: string;
      onProgress?: (info: Partial<UploadState>) => void;
      onSuccess?: (url: string, uploadId: string) => void;
      onError?: (error: string) => void;
    }) => {
      const { file, courseId, onProgress, onSuccess, onError } = options;
      ...
      try {
        // Simulate upload for demo (replace with actual implementation)
        const simulatedUrl = `https://example.com/files/${file.name}`;
        ...
        for (let i = 0; i <= 100; i += 10) {
          ...
          const eta = calculateEta(speed);

          updateState({
            progress: i,
            uploadedBytes: (file.size * i) / 100,
            speed,
            eta,
          });
        ...
        updateState({
          ...
          isUploading: false,
          progress: 100,
          uploadedUrl: simulatedUrl,
          uploadId,
        });

        onSuccess?.(simulatedUrl, uploadId);
      } catch (error) {
        const errorMessage =
    ...
    [validateFile, updateState, calculateSpeed, calculateEta]
  );

  // Pause upload
  const pauseUpload = useCallback(() => {
    updateState({ isPaused: true, isUploading: false });
  }, [updateState]);

  // Resume upload
  const resumeUpload = useCallback(() => {
    updateState({ isPaused: false, isUploading: true });
  }, [updateState]);

  // Cancel upload
  const cancelUpload = useCallback(() => {
    abortControllerRef.current?.abort();
    updateState({ ...initialState, error: 'Upload cancelled' });
  }, [updateState]);
