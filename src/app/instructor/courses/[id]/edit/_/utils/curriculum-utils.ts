  ...
  data.sections?.forEach((section: any, sectionIndex: number) => {
    ...
    if (!section.lessons || section.lessons.length === 0) {
      ...
      section.lessons?.forEach((lesson: any, lessonIndex: number) => {
        if (!lesson.name?.trim()) {
          errors.push(`Section ${sectionIndex + 1}, Lesson ${lessonIndex + 1}: Name is required`);
        }
        ...
        if (!lesson.content.title?.trim()) {
          errors.push(
            `Section ${sectionIndex + 1}, Lesson ${lessonIndex + 1}, Content : Title is required`
          );
        }

        if (lesson.content.type === 'link' && !lesson.content.url?.trim()) {
          errors.push(
            `Section ${sectionIndex + 1}, Lesson ${lessonIndex + 1}, Content : URL is required for link content`
          );
        }
