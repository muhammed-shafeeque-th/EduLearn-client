...
export const validateCurriculumData = (data: any): { isValid: boolean; errors: string[] } => {
  ...
  data.modules?.forEach((module: any, moduleIndex: number) => {
    if (!module.name?.trim()) {
      errors.push(`Module ${moduleIndex + 1}: Name is required`);
    }
    ...
    if (!module.lessons || module.lessons.length === 0) {
      ...
      module.lessons?.forEach((lesson: any, lessonIndex: number) => {
        if (!lesson.name?.trim()) {
          errors.push(`Module ${moduleIndex + 1}, Lesson ${lessonIndex + 1}: Name is required`);
        }
        ...
        if (!lesson.content.title?.trim()) {
          errors.push(
            `Module ${moduleIndex + 1}, Lesson ${lessonIndex + 1}, Content : Title is required`
          );
        }

        if (lesson.content.type === 'link' && !lesson.content.url?.trim()) {
          errors.push(
            `Module ${moduleIndex + 1}, Lesson ${lessonIndex + 1}, Content : URL is required for link content`
          );
        }
