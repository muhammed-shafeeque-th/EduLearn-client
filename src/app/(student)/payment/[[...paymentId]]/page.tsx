...
export default async function PaymentPage({ params, searchParams }: PaymentPageProps) {
  ...
  if (!orderId || typeof orderId !== 'string') {
    redirect(`${ROUTES.student.checkout}?error=${ERROR_CODES.MISSING_ORDER_ID}`);
  }
  ...
  if (!success || !order) {
    ...
    redirect(`${ROUTES.student.checkout}?error_code=${ERROR_CODES.ORDER_NOT_FOUND_OR_INVALID}`);
  }

  // Check order ownership
  if (!user?.id || user.id !== order.userId) {
    redirect(`${ROUTES.student.checkout}?error_code=${ERROR_CODES.UNAUTHORIZED_ACCESS}`);
  }

  // redirect to success if order succeeded
  if (order.status === 'succeeded') {
    redirect(`${ROUTES.payment.success}?orderId=${order.id}`);
  }

  // redirect to failure if order failed
  // if (['failed', 'cancelled'].includes(order.status)) {
  //   redirect(`${ROUTES.payment.failure}?orderId=${order.id}`);
  // }

  // // Allow only success payments
  // if (!['pending_payment', 'processing', 'created'].includes(order.status)) {
  //   redirect(`${ROUTES.student.checkout}?orderId=${order.id}&error_code=${ERROR_CODES.INVALID_ORDER_STATUS}`);
  // }
  return (
