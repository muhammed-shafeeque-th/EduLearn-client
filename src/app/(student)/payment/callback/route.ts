  ...
  try {
    ...
    if (!sessionId) {
      return NextResponse.redirect(
        new URL(`${ROUTES.payment.failure}?error=stripe_session_id_not_found`, req.nextUrl)
      );
    }
    ...
    try {
      ...
      return NextResponse.redirect(
        new URL(
          `${ROUTES.student.checkout}?error_code=${ERROR_CODES.STRIPE_VERIFICATION_FAILED}`,
          req.nextUrl
        )
      );
    }
    ...
    return NextResponse.redirect(
      new URL(
        `${ROUTES.payment.success}?orderId=${encodeURIComponent(verification.data.orderId)}`,
        req.nextUrl
      )
    );
  } catch {
    ...
    return NextResponse.redirect(
      new URL(
        `${ROUTES.student.checkout}?error_code=${ERROR_CODES.UNEXPECTED_SERVER_ERROR}`,
        req.url
      )
    );
  }
