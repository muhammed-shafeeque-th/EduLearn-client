...
export function PaymentContent({
  ...
  //       script.src = 'https:/ROUTES.student.checkout.razorpay.com/v1ROUTES.student.checkout.js';
  //       script.async = true;
  ...
  useEffect(() => {
    ...
    async function hydrateExistingOrder() {
      if (serverOrder.status === 'failed' || serverOrder.status === 'cancelled') {
        ...
        router.replace(`${ROUTES.payment.failure}?${params.toString()}`);
        return;
      }
  ...
  useEffect(() => {
    ...
    if (isSucceeded && !hasRedirectedRef.current) {
      ...
      router.replace(`${ROUTES.payment.success}?orderId=${order.id}`);
      return;
    }
  ...
  const handleCreateSession = useCallback(() => {
    ...
    createSession({
      successUrl: urlsAvailable ? `${window.location.origin}${ROUTES.payment.callback}` : undefined,
      cancelUrl: urlsAvailable ? `${window.location.origin}${ROUTES.payment.failure}` : undefined,
    });
  }, [order, createSession]);
    ...
    <div className="container mx-auto px-4 py-8 relative">
      ...
      <div className="flex items-center gap-4 mb-8">
        <Button
          variant="ghost"
          size="sm"
          onClick={() => router.push(`${ROUTES.student.checkout}/${courseId}?orderId=${orderId}`)}
