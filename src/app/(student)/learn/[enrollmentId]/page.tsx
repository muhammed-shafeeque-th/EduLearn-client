...
export async function generateMetadata({ params }: EnrollmentPageProps): Promise<Metadata> {
  ...
  await authGuard({
    // roles: ['student'],
    redirectTo: `${ROUTES.auth.login}?next=${ROUTES.student.learn(enrollmentId)}`,
  });
...
export default async function EnrollmentPage({ params, searchParams }: EnrollmentPageProps) {
  ...
  const currentUser = await authGuard({
    ...
    // },
    // onUnauthorized() {
    //   return redirect(`${ROUTES.public.courses.root}?error_code=${ERROR_CODES.NOT_ENROLLED}`);
    // },
  });
  ...
  if (!enrollment || !success) {
    return redirect(`${ROUTES.public.courses.root}?error_code=${ERROR_CODES.ENROLLMENT_NOT_FOUND}`);
    // notFound();
  }
