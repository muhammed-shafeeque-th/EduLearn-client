...
export function CheckoutContent({ existingOrderId, courseId, checkoutType }: CheckoutContentProps) {
  ...
  useEffect(() => {
    if (isOrderCreated && order?.id && !hasRedirectedRef.current) {
      ...
      setTimeout(() => {
        router.push(`${ROUTES.payment.root}?orderId=${order.id}`);
      }, 0);
    }
  ...
  const handleProceedToPayment = () => {
    ...
    if (
      ...
      router.push(`${ROUTES.payment.root}?orderId=${orderData.orderId}`);
      return;
    }
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Button
              ...
              onClick={() => router.push(ROUTES.public.courses.root)}
            >
              Browse Courses
            </Button>
            {checkoutType === 'course' && (
