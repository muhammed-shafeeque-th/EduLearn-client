...
export async function applyCouponCode(orderId: string, couponCode: string) {
  try {
    ...
    revalidatePath(ROUTES.student.checkout);
...
export async function verifyStripePayment(verificationData: PaymentVerificationData) {
  try {
    ...
    if (result.success) {
      redirect(`${ROUTES.student.checkout}/success?orderId=${verificationData.orderId}`);
    }
...
export async function proceedToPayment(orderId: string, courseId?: string) {
  const basePath = courseId ? `${ROUTES.student.checkout}/${courseId}` : ROUTES.student.checkout;
  redirect(`${basePath}${ROUTES.payment.root}?orderId=${orderId}`);
}

export async function redirectToFailure(orderId: string, error?: string) {
  const params = new URLSearchParams({ orderId });
  if (error) params.set('error', error);
  redirect(`${ROUTES.student.checkout}/failure?${params.toString()}`);
}
