...
import { InstructorsSkeleton } from './loading-skeletons';
...
const InstructorsSection = () => {
  ...
  const { instructors: instructorsRes, isLoading } = useInstructors(
    { page: 1, pageSize: 5 },
    { enabled: true }
  );
  ...
  const mappedInstructors = useMemo(() => {
    return (
      instructorsRes?.map((inst) => ({
        id: inst.id,
        name: inst.username,
        title: inst.headline || inst.expertise || 'Top Instructor',
        avatar:
          inst.avatar ||
          'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=150&h=150&fit=crop&crop=face',
        rating: inst.rating || 0,
        students: inst.totalStudents || 0,
      })) || []
    );
  }, [instructorsRes]);

  const instructors = mappedInstructors.length >= 5 ? mappedInstructors : demoInstructors;
      ...
      <div className="container mx-auto">
        ...
        >
          ...
          <Button
            ...
            onClick={() => router.push(ROUTES.public.instructors.root)}
          >
            See All
