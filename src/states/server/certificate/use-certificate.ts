...
export function useEnrollmentCertificate(enrollmentId: string) {
  ...
  const certificateQuery = useQuery({
    ...
    queryFn: async () => {
      try {
        const result = await enrollmentService.getCertificateByEnrollmentId(enrollmentId);
...
export function useCertificate(certificateId: string) {
  const certificateQuery = useQuery({
    queryKey: QUERY_KEYS.certificates.byId(certificateId),
    queryFn: async () => {
      try {
        const result = await enrollmentService.getCertificate(certificateId);
        if (!result.success || !result.data) {
          throw new Error(result.message);
        }
        return result.data;
